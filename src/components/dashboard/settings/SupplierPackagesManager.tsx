...
import { getSupplierPackages, createSupplierPackage, updateSupplierPackage, deleteSupplierPackage } from '@/lib/actions/supplier-packages'
...
interface SupplierPackagesManagerProps {
    onOpenChange?: (open: boolean) => void
}

export function SupplierPackagesManager({ onOpenChange }: SupplierPackagesManagerProps) {
    ...
    const handleSave = async () => {
        ...
        if (isCreating) {
            const res = await createSupplierPackage(payload)
            if (res.success) {
            ...
            const res = await updateSupplierPackage(editingId, payload)
            if (res.success) {
    ...
    const handleDelete = async (id: string, name: string) => {
        const confirmed = await confirm(
            'ספקים המשוייכים לחבילה זו ינותקו ממנה.',
            'האם אתה בטוח שברצונך למחוק את החבילה?'
        )

        if (confirmed) {
            const res = await deleteSupplierPackage(id)
            if (res.success) {
    ...
    return (
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
                <DialogTitle>ניהול חבילות ספקים / קטגוריות</DialogTitle>
            </DialogHeader>
            ...
            <div className="space-y-6 mt-4">
                ...
                {isCreating ? (
                    ...
                    <Button
                        ...
                        className="w-full border-2 border-dashed border-gray-300 hover:border-gray-400 hover:bg-gray-50 dark:border-slate-700 dark:hover:bg-slate-800 text-gray-500 dark:text-gray-400 h-12"
                        variant="ghost"
                    >
