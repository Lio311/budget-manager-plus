...
export async function getNetWorthHistory(type: 'PERSONAL' | 'BUSINESS' = 'PERSONAL') {
    try {
        ...
        for (const budget of budgets) {
            ...
            const totalIncome = budget.incomes.reduce((sum: number, item: { amount: number }) => sum + item.amount, 0)

            // Calculate total expenses (Expenses + Bills + Debt Monthly Payments)
            const totalExpenses = budget.expenses.reduce((sum: number, item: { amount: number }) => sum + item.amount, 0)
            ...
            const totalBills = budget.bills.reduce((sum: number, item: { amount: number }) => sum + item.amount, 0)

            // For debts, only monthly payment counts as immediate outflow
            const totalDebtPayments = budget.debts.reduce((sum: number, item: { monthlyPayment: number }) => sum + item.monthlyPayment, 0)
