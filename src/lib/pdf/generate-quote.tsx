...
export async function generateQuotePDF({ quoteId, userId }: GenerateQuotePDFParams): Promise<{ buffer: Buffer, filename: string }> {
    try {
        ...
        // Generate PDF
        const unsignedBuffer = await renderToBuffer(<InvoiceTemplate data={quoteData} />)
        const buffer = await signPdfBuffer(unsignedBuffer, {
            signerName: quoteData.businessName || 'Kesefly',
            reason: 'Original quote document generated by Kesefly',
        })
